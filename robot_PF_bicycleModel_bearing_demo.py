...
class robot:
    ...
    def move(self, motion, tolerance = 0.001):
        ...
        if motion[0] > max_steering_angle:
        ...
        if abs(beta) < tolerance:
            result.x = self.x + cos(self.orientation) * distance
            result.y = self.y + sin(self.orientation) * distance
...
def particle_filter(motions, measurements, N=800): # I know it's tempting, but don't change N!
    # --------
    #
    # Make particles
    # 
